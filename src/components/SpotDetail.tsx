...
export default function SpotDetail({ spot, isVisited, isInTrip, onClose, onGetDirections, onToggleVisited, onToggleTrip }: SpotDetailProps) {
  ...
  return (
    ...
    >
      {/* Floating card */}
      <div className="w-full max-w-sm overflow-hidden rounded-3xl bg-white shadow-2xl flex flex-col" style={{ maxHeight: "85vh" }}>
        ...
        <div className="relative h-44 shrink-0 overflow-hidden bg-stone-100">
          <img
            ...
            onError={(e) => {
              (e.target as HTMLImageElement).src = `https://placehold.co/400x176/${categoryColors[spot.category].replace("#", "")}/ffffff?text=${encodeURIComponent(spot.name.split(" ")[0])}`;
            }}
          />
          ...
          <div className="absolute bottom-3 left-3 flex items-center gap-2">
            ...
            <span className={`flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-bold ${isOpen ? "bg-emerald-500 text-white" : "bg-black/50 text-white"}`}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          <h2 className="text-lg font-black leading-tight text-stone-900">{spot.name}</h2>
          ...
          <p className="mt-2.5 text-xs leading-relaxed text-stone-500 line-clamp-3">{spot.description}</p>

          {/* Info row */}
          <div className="mt-3 grid grid-cols-3 gap-1.5">
            <div className="col-span-2 rounded-xl bg-stone-50 px-3 py-2">
              <p className="text-[8px] font-bold uppercase tracking-widest text-stone-400">Hours</p>
              <p className="mt-0.5 text-[11px] font-semibold text-stone-700 leading-snug">{spot.hours}</p>
            </div>
            <div className={`rounded-xl px-3 py-2 ${spot.admission === "Free" ? "bg-emerald-50" : "bg-blue-50"}`}>
              <p className={`text-[8px] font-bold uppercase tracking-widest ${spot.admission === "Free" ? "text-emerald-400" : "text-blue-400"}`}>Admission</p>
              <p className={`mt-0.5 text-[11px] font-bold leading-snug ${spot.admission === "Free" ? "text-emerald-700" : "text-blue-700"}`}>{spot.admission}</p>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-bold text-white transition-all active:scale-95"
                style={{ backgroundColor: categoryColors[spot.category] }}
              >
                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <a
                ...
                className="flex items-center justify-center gap-1.5 rounded-xl bg-[#4285F4] py-2.5 text-xs font-bold text-white transition-all active:scale-95"
              >
